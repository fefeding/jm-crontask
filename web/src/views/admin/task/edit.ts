...
export default class Edit extends Vue {
    ...
    get timeValue() {        
        ...
        else if(this.taskData.timeSize !== TaskTimeSize.crontab && !/^\d+$/.test(this.taskData.timeValue)) {
    ...
    async save() {
        ...
        // 定时规则表达式校验
        if(this.taskData.timeSize === TaskTimeSize.crontab && (this.taskData.timeValue.split(' ').length > 7 || this.taskData.timeValue.split(' ').length < 5)) {
            this.$message.error('请输入正确的定时规则表达式！');
            return false;
        }
